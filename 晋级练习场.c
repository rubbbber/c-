...
List*initWordList()
{
    List*root = NULL;//必须置为NULL否则无法判断其位置
    List*current;
    List*new = root;
    int i = 0;
    for(i=0;i<26;i++)
...
int insertword(Con**current,char*word)
{
    Con*next;
    while((next=*current)!=NULL&&strcmp(next->word,word)<0)
    {
        current = &next->next;
    }
    Con*New = (Con*)malloc(sizeof(Con));
    if(New == NULL)
    {
        return FLASE;
    }
...
int insertWordList(List*L,char*word)
{
    int new;
    while (L!=NULL)
    {
        if(L->first == word[0])//找到首字母
        {
            new = insertword(&L->bwd,word);//创建单词&&寻找插入位置
            if(new != 1)
                return FLASE;
            else
                return TRUE;
